...
PARAMS = [
    ("api_key", "itrack_server.api_key"),
    ("api_url", "itrack_server.api_url"),
]

class SasSettings(models.TransientModel):
    _name = 'itrack_fleet.settings'
    ...
    def import_cars_fms(self):
        """ Import cars from iTrack
        """
        self.server_params = self.get_default_params(None)
        if not self.server_params.get("api_key"): return
        url = self.server_params.get("api_url") + "GetVehicles.ashx"
        response_body = requests.post(url, params={'accessToken': self.server_params.get("api_key"), 'format':'json'})
        ...
        #import drivers
        url = self.server_params.get("api_url") + "GetDrivers.ashx"
        response_body = requests.post(url, params={'accessToken': self.server_params.get("api_key"), 'format':'json'})
    ...
    def import_gps_fms(self, days_to_process=1):
        ...
        for faz_date in self.daterange(start_date, end_date):
            for car in cars:
                ...
                url = self.server_params.get("api_url") + "GetVehicleTracks.ashx"
                try:
                    query_params = {'accessToken': self.server_params.get("api_key"), 'format':'json',
                    ...
                    _logger.exception("Exception processing GetVehicleTracks for url %s and query %s" % (url, query_params))
