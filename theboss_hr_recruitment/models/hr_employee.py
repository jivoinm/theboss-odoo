...
class hr_employee(models.Model):
    ...
    attachment_ids = fields.One2many('ir.attachment', 'res_id', domain=[('res_model', '=', 'hr.employee')], string='Attachments', ondelete='set null')
    ...
    def action_get_attachment_tree_view(self):
        ...
        action['search_view_id'] = (self.env.ref('theboss_hr_recruitment.ir_attachment_view_search_inherit_hr_employee').id, )
        return action

    @api.multi
    def action_generate_documents(self):
        self.ensure_one()
        attachment_ids = self.env['hr.document'].generate_document_attachements(self, 'hr.employee')
