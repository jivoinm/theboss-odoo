...
class HRDocument(models.Model):
    ...
    def create_action(self):
        ...
        for template in self:
            ...
            button_name = _('Print (%s)') % template.name
            action = ActWindowSudo.create({
                'name': button_name,
                'string': button_name,
                'model': src_obj,
                'report_type': 'qweb-pdf',
                'report_name': 'theboss_hr.report_print_hr_doc',
                'attachment_use': True,
                'context': "{'template_id' : %d, 'res_id': active_id}" % (template.id),
            ...
            ir_value = IrValuesSudo.create({
                ...
                'value': "ir.actions.report.xml,%s" % action.id})
            template.write({
                'ref_ir_act_report': action.id,
    ...
    # @http.route('/web/binary/download_hr_document', type='http', auth="public")
    # @serialize_exception
    @api.multi
    ...
    @api.model
    def get_document_template(self, res_model, res_id):
        '''
        ...
        '''
        if not res_id:
            raise UserError(_("Creating document may only be done by one at a time."))
...
class HRDocumentReport(models.AbstractModel):
    _name = 'report.theboss_hr.print_hr_doc'
    @api.model
    def render_html(self, docids, data=None):
        report_obj = self.env['report']
        report = report_obj._get_report_from_name('module.report_name')
        res_ids = self.context['res_ids']
        docargs = {
            'doc_ids': docids,
